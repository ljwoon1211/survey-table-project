...
export function TableValidationEditor({
  ...
  const [rules, setRules] = useState<TableValidationRule[]>(
    question.tableValidationRules || []
  );
  ...
  const validationTypes: { value: TableValidationType; label: string; description: string }[] = [
    {
      ...
      label: "독점 체크 (특정 행만)",
      description: "지정한 행만 체크되고 다른 행은 체크 안됨",
    },
    {
      value: "any-of",
      label: "하나라도 체크",
      description: "지정한 행 중 하나라도 체크됨",
    },
    {
      value: "all-of",
      label: "모두 체크",
      description: "지정한 행이 모두 체크됨",
    },
    {
      value: "none-of",
      label: "모두 미체크",
      description: "지정한 행이 모두 체크 안됨",
    },
    {
      value: "required-combination",
      label: "필수 조합",
      description: "지정한 행들이 모두 체크되어야 함",
    },
  ];
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">
                규칙 {index + 1}
                {rule.description && <span className="text-sm font-normal ml-2">- {rule.description}</span>}
              </CardTitle>
              <Button
                variant="ghost"
                size="sm"
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <select
                id={`type-${rule.id}`}
                value={rule.type}
                onChange={(e) => updateRule(rule.id, { type: e.target.value as TableValidationType })}
            ...
            {rule.conditions.checkType !== "checkbox" && 
             ...
             rule.conditions.checkType !== "select" && (
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-gray-500">
                  입력 필드에 이 값들 중 하나가 입력되었는지 확인합니다. 비워두면 값이 있는지만 확인합니다.
                </p>
              </div>
            )}
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto border border-gray-200 rounded-md p-3">
                  {(() => {
                    ...
                    return options.map((option) => {
                      const isSelected = rule.conditions.expectedValues?.includes(option.value) || false;
