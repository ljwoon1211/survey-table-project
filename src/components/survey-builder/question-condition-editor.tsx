    ...
    <div className="space-y-4">
      ...
      <>
          ...
          {conditionGroup?.conditions.map((condition, index) => {
            ...
            const conditionName = conditionNames[condition.id] !== undefined 
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto border border-gray-200 rounded-md p-3">
                          {sourceQuestion.tableRowsData?.map((row, rowIndex) => {
                            const colIndex = condition.tableConditions?.cellColumnIndex;
                            const mergeInfo = getRowMergeInfo(row.id, sourceQuestion?.tableRowsData, colIndex);
                            const isSelected = condition.tableConditions?.rowIds.includes(row.id) || false;
                    ...
                    <div className="space-y-3 pt-3 border-t border-gray-200">
                        ...
                        <Switch
                          ...
                          onCheckedChange={(checked) => {
                            if (checked) {
                              ...
                              updateCondition(condition.id, { additionalConditions: undefined });
                            }
                          }}
                        />
                      </div>
                      ...
                      {condition.additionalConditions && (
                        <div className="space-y-3 pl-4 border-l-2 border-blue-200">
                          {/* 추가 조건 열 인덱스 */}
                          <div className="space-y-2">
                            <Label htmlFor={`additional-col-${condition.id}`}>확인할 열 인덱스</Label>
                            <Input
                          ...
                          {/* 추가 조건 체크 타입 */}
                          <div className="space-y-2">
                            <Label htmlFor={`additional-check-type-${condition.id}`}>체크 타입</Label>
                            <select
                        ...
                        <div className="space-y-2">
                          <div className="max-h-48 overflow-y-auto border border-gray-200 rounded-md p-3 space-y-2">
                            {sourceQuestion.options.map((option) => {
                              const isSelected = (condition.requiredValues || []).includes(option.value);
                              return (
                                <div key={option.id} className="flex items-center gap-2">
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      updateCondition(condition.id, { requiredValues: newValues.length > 0 ? newValues : [] });
                                    }}
                                    className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                                  />
                                  <label
                          ...
                          {(condition.requiredValues || []).length === 0 && (
                            <p className="text-xs text-red-600">최소 1개 이상의 옵션을 선택해주세요</p>
                          )}
                        </div>
                      ) : (
